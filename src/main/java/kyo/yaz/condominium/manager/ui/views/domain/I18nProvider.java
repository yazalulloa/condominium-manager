...
@AllArgsConstructor
    ...
    public DatePicker.DatePickerI18n datePickerI18n(String zoneId) {
        ...
        if (i18n == null && zoneId.equals("America/Caracas")) {
            ...
            final var months = Arrays.stream(Month.values()).map(Enum::name).map(translationProvider::translate).map(str -> CaseFormat.UPPER_UNDERSCORE.to(CaseFormat.UPPER_CAMEL, str)).toList();

            final var weekDays = Arrays.stream(DayOfWeek.values()).map(Enum::name).map(translationProvider::translate).map(str -> CaseFormat.UPPER_UNDERSCORE.to(CaseFormat.UPPER_CAMEL, str)).toList();
