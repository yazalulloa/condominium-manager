...
public class BuildingView extends BaseVerticalLayout {
  ...
  private Component description(Building building) {
    ...
    final var currenciesToShowAmountToPay = new Span(
        Labels.Building.SHOW_PAYMENT_IN_CURRENCIES + ": " + building.currenciesToShowAmountToPay().toString());
    ...
    final Component component =
        building.fixedPay() ? new Span(ConvertUtil.format(building.fixedPayAmount(), building.mainCurrency()))
            : IconUtil.cross();
    ...
    final var description = new Div(mainCurrency, debtCurrency, currenciesToShowAmountToPay, extraCharges, fixedPay,
        receiptEmailFrom, roundUpPayments, amountOfApts);
  ...
  private Single<List<Building>> listOfBuildings() {
    return service.list(null)
        .doOnSubscribe(d -> {
          uiAsyncAction(() -> {
            progressLayout.setProgressText("Buscando edificios");
