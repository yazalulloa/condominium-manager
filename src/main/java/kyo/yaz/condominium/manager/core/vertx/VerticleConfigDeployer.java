    ...
    final List<Future<?>> futures = verticles.stream()
        .map(verticleRecord -> {
          ...
          if (jsonObject != null) {
            ...
            final var shouldDeploy =
                verticleRecord.oldConfig() == null || !verticleRecord.oldConfig().equals(jsonObject);

            if (shouldDeploy) {

              final var oldVerticleId = verticleRecord.verticleId();

              return vertx.deployVerticle(verticleRecord.verticle(),
                      new DeploymentOptions().setInstances(verticleInstances).setConfig(jsonObject))
