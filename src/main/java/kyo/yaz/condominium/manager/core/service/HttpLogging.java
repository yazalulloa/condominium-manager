...
public class HttpLogging {
    ...
    public void logRequest(long requestCounter, boolean prettyJson, HttpClientRequest clientRequest, HttpRequest<Buffer> httpRequest) {
        ...
        try {
            ...
            final var logConfig = Optional.ofNullable(clientRequest.requestLogConfig()).orElseGet(() -> HttpLogConfig.builder().build());
            ...
            if (method.equals(HttpMethod.PUT) || method.equals(HttpMethod.POST)) {
                ...
                if (clientRequest.body() != null && logConfig.showBody()) {
    ...
    public void logResponse(long requestCounter, boolean prettyJson, HttpClientRequest clientRequest, HttpClientResponse response, long responseTime, Throwable throwable, HttpLogConfig logConfig) {
        ...
        try {
            ...
            if (response != null) {
                ...
                builder.append(requestCounter).append(direction).append(response.statusCode()).append(" ").append(response.httpMethod())
                ...
                if (response.cookies() != null && !response.cookies().isEmpty()) {
                    builder.append(requestCounter).append(direction).append("Cookies: ").append(String.join(",", response.cookies())).append("\n");
                }
                ...
                builder.append(requestCounter).append(direction).append(clientRequest.httpMethod()).append(" ").append(uri).append("\n")
                        .append(requestCounter).append(direction).append(responseTime).append("ms").append("\n");
            }
